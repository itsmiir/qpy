...
class Quantity:
    ...
    def __init__(self, value: ArrayLike, unit: Unit | Quantity, digits: int=0) -> Quantity:
    ...
    def __str__(self):
        if self.digits > 0:
            ...
            val = self.value
        return str(val)+" "+simplify(self.unit)
    def __float__(self):
        return self.value
    def __int__(self):
    ...
    def terms_of(self, other,rnd: int=-1)->str:
        if type(other) == Unit:
            if (other.offset != 0):
                return self.terms_of(1)
            return self.terms_of(1*other, rnd)
        elif type(other) == Quantity:
            ...
            if rnd > -1:
                val = round(val, rnd)
            return (str(val)+" "+other.unit.symbol)
